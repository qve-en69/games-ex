<template>
  <div class="justify-content-center text-center preform">
    <form class="form-signin">
      ...
      <div class="px-3">
        <Button @click.prevent="Sumbit" class="mb-1 button-submit" type="submit">
...
export default {
  ...
  methods: {
    ...mapActions('registration', ['registerFast']),
    activeClick (icon) {
      console.log(icon)
      ...
      if (!this.$v.$invalid) {
        this.$recaptchaLoaded().then(() => {
          this.$recaptcha('login').then((token) => {
            this.registerFast({ rToken: token, nick: this.nick, avatar: this.active_icon })
              .then(data => {
                this.$emit('update:component', 'FastRegistrationSuccess')
              }).catch(e => {
        ...
        this.$noty.error('Invalid nickname')
      }
    }
  }
}
